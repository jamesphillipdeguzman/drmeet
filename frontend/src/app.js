...
async function renderPatients() {
    mainContent.innerHTML = '<h2>Patients</h2><div class="feedback">Loading...</div>';
    try {
          ...
          ${patients.map(p => `
            <tr>
              ...
              <td>
                ...
                <button onclick="window.deletePatient('${p._id}')">Delete</button>
              </td>
            </tr>
          `).join('')}
        </tbody>
...
function showPatientForm(editId = null) {
  ...
  `;
    window.closePatientForm = () => { modal.style.display = 'none'; };
    ...
    form.onsubmit = async e => {
        ...
        try {
            const res = await fetch(`${API_BASE}/patients${editId ? '/' + editId : ''}`, {
...
function editPatient(id) { showPatientForm(id); }
...
async function renderDoctors() {
    mainContent.innerHTML = '<h2>Doctors</h2><div class="feedback">Loading...</div>';
    try {
          ...
          ${doctors.map(d => `
            <tr>
              ...
              <td>
                ...
                <button onclick="window.deleteDoctor('${d._id}')">Delete</button>
              </td>
            </tr>
          `).join('')}
        </tbody>
...
function showDoctorForm(editId = null) {
  ...
  `;
    window.closeDoctorForm = () => { modal.style.display = 'none'; };
    ...
    form.onsubmit = async e => {
        ...
        try {
            const res = await fetch(`${API_BASE}/doctors${editId ? '/' + editId : ''}`, {
                method: editId ? 'PUT' : 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function editDoctor(id) { showDoctorForm(id); }
...
async function renderAppointments() {
    mainContent.innerHTML = '<h2>Appointments</h2><div class="feedback">Loading...</div>';
    try {
        ...
        <tbody>
          ${appointments.map(a => `
            <tr>
              ...
              <td>${a.status || ''}</td>
              <td>
                <button onclick="window.editAppointment('${a._id}')">Edit</button>
                <button onclick="window.deleteAppointment('${a._id}')">Delete</button>
              </td>
            </tr>
          `).join('')}
        </tbody>
...
function showAppointmentForm(editId = null) {
  ...
  `;
    window.closeAppointmentForm = () => { modal.style.display = 'none'; };
    ...
    form.onsubmit = async e => {
        ...
        try {
            const res = await fetch(`${API_BASE}/appointments${editId ? '/' + editId : ''}`, {
                method: editId ? 'PUT' : 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function editAppointment(id) { showAppointmentForm(id); }
...
async function renderUsers() {
    mainContent.innerHTML = '<h2>Users</h2><div class="feedback">Loading...</div>';
    try {
...
function showUserForm(editId = null) {
  ...
  `;
    window.closeUserForm = () => { modal.style.display = 'none'; };
    ...
    form.onsubmit = async e => {
        ...
        try {
            const res = await fetch(`${API_BASE}/users${editId ? '/' + editId : ''}`, {
                method: editId ? 'PUT' : 'POST',
                headers: { 'Content-Type': 'application/json' },
